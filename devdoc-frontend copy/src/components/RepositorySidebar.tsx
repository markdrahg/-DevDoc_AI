...
export function RepositorySidebar({ onBack }: RepositorySidebarProps) {
  ...
  const renderTree = (nodes: FileNode[], depth = 0) => {
    return nodes.map((node) => (
      <div key={node.name}>
        <div
          className={`flex items-center gap-2.5 py-2 px-2.5 rounded-md hover:bg-accent/30 cursor-pointer group transition-colors`}
      ...
      <div className="h-full flex flex-col">
        ...
        {onBack && (
          <div className="px-4 py-3 border-b border-border/40">
            <button
              onClick={onBack}
              className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-accent/50 transition-colors text-sm text-muted-foreground hover:text-foreground w-full"
            >
